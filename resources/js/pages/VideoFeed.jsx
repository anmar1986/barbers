...
const VideoFeed = () => {
    ...
    // Fetch videos (initial or load more)
    const fetchVideos = useCallback(async (pageNum = 1, append = false) => {
        ...
        try {
            const response = activeTab === 'feed'
                ? await videoAPI.getFeed({ page: pageNum, limit: VIDEOS_PER_PAGE })
                : await videoAPI.getTrending({ page: pageNum, limit: VIDEOS_PER_PAGE });
